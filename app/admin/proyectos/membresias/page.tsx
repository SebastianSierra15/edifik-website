...
import MembershipModal from "@/app/ui/projects/memberships/membershipModal";
...
export default function MembershipsPage() {
  ...
  const fetchMemberships = async () => {
    try {
      const response = await axios.get(
        `/api/projects/metadata/memberships?page=${currentPage}&pageSize=${entriesPerPage}&searchTerm=${searchTerm}`
      );
  ...
  const validateFields = () => {
    ...
    if (updateMembership.maxProjects <= 0) {
      tempErrors.maxProjectsError = "Debe haber al menos 1 propiedad.";
      isValid = false;
    }
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put("/api/projects/metadata/memberships", updateMembership);
      setModalOpen(false);
      fetchMemberships();
    } catch (error) {
  ...
  const headers: { label: string; key: keyof Membership }[] = [
    ...
    { label: "Proyectos Destacados", key: "projectsFeatured" },
    { label: "Max. Proyectos", key: "maxProjects" },
