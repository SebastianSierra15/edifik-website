...
export function useS3ImagesApi() {
  ...
  const uploadImages = useCallback(
    async (
      ...
      try {
        setStatus("Procesando y subiendo imÇ­genes...");
        setProgress(0);
        ...
        const processedMedia = await Promise.allSettled(
          media.map(async (item, index) => {
            ...
            if (!processedFile) {
              console.warn(
                `ÐY"û Se omitiÇü la subida de ${item.file} porque es una URL existente.`
              );
              return null;
            }
        ...
        if (failedUploads.length > 0) {
          console.warn(
            "ƒsÿ‹÷? Algunas imÇ­genes no se pudieron subir:",
            failedUploads
          );
          setStatus("Algunas imÇ­genes no se pudieron subir.");
        } else {
          setStatus("ImÇ­genes subidas satisfactoriamente.");
        }

        return successfulUploads;
      } catch (error) {
        console.error("ÐYsù Error al subir imÇ­genes:", error);
        setStatus("Error al subir imÇ­genes.");
        throw error;
      } finally {
        setProgress(100);
      }
  ...
  const deleteImages = useCallback(async (imageUrls: string[]) => {
    if (imageUrls.length === 0) {
      console.warn("ƒsÿ‹÷? No se proporcionaron imÇ­genes para eliminar.");
      return;
    }

    try {
      setStatus("Eliminando imÇ­genes...");
      setProgress(0);
      ...
      const deleteResults = await Promise.allSettled(
        imageUrls.map(async (url, index) => {
          ...
          if (!s3Key) {
            console.warn(
              `ƒ?O No se pudo extraer la clave de S3 de la URL: ${url}`
            );
            return null;
          }
          ...
          if (!response.ok) {
            throw new Error(`ƒ?O Error al eliminar la imagen: ${s3Key}`);
          }
      ...
      if (failedDeletes.length > 0) {
        console.warn(
          "ƒ?O Algunas imÇ­genes no se pudieron eliminar:",
          failedDeletes
        );
        setStatus("Algunas imÇ­genes no se pudieron eliminar.");
      } else {
        setStatus("ImÇ­genes eliminadas satisfactoriamente.");
      }

      return successfulDeletes;
    } catch (error) {
      console.error("ƒ?O Error al eliminar imÇ­genes:", error);
      setStatus("Error al eliminar imÇ­genes.");
      throw error;
    } finally {
      setProgress(100);
    }
